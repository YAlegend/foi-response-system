...
def send_reminder(db: Session, req: FOIRequest, actor: str = "system") -> dict:
    ...
    recipients = (_recipients_for(dept, s, _recipient_map(s)) if dept
    ...
    body = (f"Reminder: FOI case {req.reference} is {when}.\n\n"
            f"Subject: {req.subject}\n"
            f"Scheme: {scheme}\n"
            f"Stage: {req.stage}\n"
            f"Responsible: {dept or 'Unassigned'}\n"
    ...
    audit.log(db, req, actor, "reminder_sent",
              detail=(f"to {dept or 'central IG'} "
                      f"<{', '.join(recipients) or 'no recipients configured'}> — {when}"))
    db.commit()
    return {
        "ok": True, "reference": req.reference, "department": dept or "Unassigned",
